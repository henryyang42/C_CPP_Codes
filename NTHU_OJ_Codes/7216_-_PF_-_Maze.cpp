...
int find(int ii, int jj, char temp[MAX][MAX], int n, int m, char c){
	...
	while(!i.empty() && map[i.front()][j.front()] != c){	
		for(int k = 0; k < 8; k++)
			...
			j.front() + dj[k] >= 0 && j.front() + dj[k] < m && temp[i.front() + di[k]][j.front() + dj[k]] != '#')
				i.push(i.front() + di[k]), j.push(j.front() + dj[k]), ct.push(ct.front() + 1), temp[i.front()+ di[k]][j.front() + dj[k]] = '#';
		i.pop() ,j.pop(), ct.pop();		
	}
	if(ct.empty())
		return -1;
